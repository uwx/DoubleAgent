...
namespace Sample1
{
	public partial class Sample1 : Form
	{
		...
		DoubleAgentSvr.DaCharacter mDaServerChar;
		...
		private void LoadDaServerChar_Click(object sender, EventArgs e)
		{
			...
			if (mDaServerChar == null)
			{
				...
				mDaServerChar = (DoubleAgentSvr.DaCharacter)lCharacter;
				if (mDaServerChar != null)
				{
		...
		{
			DoubleAgentSvr.DaPropertySheet lServerProps;

			if (mDaServer == null)
			{
				mDaServer = new DoubleAgentSvr.DaServer();
			}
			lServerProps = (DoubleAgentSvr.DaPropertySheet)mDaServer;
			lServerProps.SetVisible(1);
		}
