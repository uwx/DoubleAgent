...
namespace Microsoft.Ddue.Tools
{
	class MSHCPackager
	{
		...
		{
			Option v_operation = null;
			Option v_extractOperation;
			Option v_saveOperation;
			Option v_copyOperation;
			...
			foreach (String v_argument in v_parseResults.UnusedArguments)
			{
				if (v_operation == v_extractOperation)
				{
					...
					{
						Console.WriteLine (String.Format ("Too many arguments '{0}'.", v_argument));
						return (1);
					}
				}
			...
			if (v_operation == v_extractOperation)
			{
				...
				{
					Console.WriteLine (String.Format ("{0} -> {1}", v_sourcePackage.PackagePath, v_targetDirectory));
				...
				{
					Console.WriteLine (String.Format ("Unable to open {0}", v_mshcFile));
					return (1);
				}
			}
			else if (v_operation == v_saveOperation)
			...
			else if (v_operation == v_copyOperation)
			{
				...
				Console.WriteLine (String.Format ("{0} -> {1}", Path.GetDirectoryName(v_manifestFile), v_targetDirectory));
