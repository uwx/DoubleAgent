		...
		<tr>
			...
			<td>
				<xsl:call-template name="typeIcon">
					<xsl:with-param name="typeVisibility"
  ...
  -->
		...
		<xsl:call-template name="section">
			...
			<xsl:with-param name="content">
				<xsl:call-template name="namespaceList">
					<xsl:with-param name="listSubgroup"
													select="$listSubgroup" />
				</xsl:call-template>
			</xsl:with-param>
		</xsl:call-template>
	...
	<xsl:template name="namespaceList">
		...
		<table id="typeList"
			...
			<xsl:apply-templates select="element[apidata/@subgroup=$listSubgroup]"
													 mode="namespace">
				<xsl:sort select="@api" />
			</xsl:apply-templates>
		</table>
	...
	<xsl:template name="IsMemberSupportedOnXna">
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:variable name="platformFilterExcludesXna"
	...
	<xsl:template name="IsMemberSupportedOnCf">
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:variable name="platformFilterExcludesCF"
	...
	<xsl:template name="IsMemberSupportedOnSilverlight">
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:variable name="platformFilterExcludesSilverlight"
											select="boolean( platforms and not(platforms[platform[.='SilverlightPlatforms']]) )" />
...
-->
				<td>
					<xsl:call-template name="memberIcons">
						...
						<xsl:with-param name="staticMember"
														select="normalize-space($staticMember)" />
						<xsl:with-param name="supportedOnXna"
														select="normalize-space($supportedOnXna)"/>
						<xsl:with-param name="supportedOnCf"
														select="normalize-space($supportedOnCf)"/>
						<xsl:with-param name="supportedOnSilverlight"
														select="normalized-space($supportedOnSilverlight)" />
						<xsl:with-param name="supportedOnSilverlightMobile"
														select="normalize-space($supportedOnSilverlightMobile)" />
					</xsl:call-template>
				</td>
				<td>
					<xsl:choose>
	...
	<xsl:template name="typeIcon">
		...
		<img>
			...
			<includeAttribute name="alt"
												item="{concat($typeVisibility,$typeSubgroup,'AltText')}" />
			<includeAttribute name="title"
												item="{concat($typeVisibility,$typeSubgroup,'AltText')}" />
		</img>
		...
		<img>
			...
			<xsl:choose>
				<xsl:when test="apidata/@subsubgroup">
					<includeAttribute name="alt"
														item="{concat($memberVisibility,apidata/@subsubgroup,'AltText')}" />
					<includeAttribute name="title"
														item="{concat($memberVisibility,apidata/@subsubgroup,'AltText')}" />
				</xsl:when>
				<xsl:otherwise>
					<includeAttribute name="alt"
														item="{concat($memberVisibility,$memberSubgroup,'AltText')}" />
					<includeAttribute name="title"
														item="{concat($memberVisibility,$memberSubgroup,'AltText')}" />
				</xsl:otherwise>
			</xsl:choose>
	...
	<xsl:template name="processVersions">
		...
		<xsl:choose>
			<xsl:when test="versions and $frameworkGroup">
				<xsl:for-each select="versions">
					...
					<xsl:if test="not($platformFilterExcluded) and count(.//version) &gt; 0">
						...
						<xsl:call-template name="processVersions">
							<xsl:with-param name="frameworkGroup"
															select="false()"/>
						</xsl:call-template>
					</xsl:if>
				</xsl:for-each>
			...
			<xsl:otherwise>
				...
				<xsl:variable name="supportedCount"
