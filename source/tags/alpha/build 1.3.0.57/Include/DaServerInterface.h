...
__if_not_exists(IDaSvrTTSEnginePtr)
{
	...
	public:
		...
		short _get_MajorVersion () {short lMajorVersion=0,lMinorVersion=0; if (_DASVRPTR_CHKNULL) GetInterfacePtr()->GetVersion(&lMajorVersion, &lMinorVersion); return lMajorVersion;}
		short _get_MinorVersion () {short lMajorVersion=0,lMinorVersion=0; if (_DASVRPTR_CHKNULL) GetInterfacePtr()->GetVersion(&lMajorVersion, &lMinorVersion); return lMinorVersion;}
	};
}

/////////////////////////////////////////////////////////////////////////////

__if_not_exists(IDaSvrTTSPrivatePtr)
{
	_DASVRPTR_DECLARE(IDaSvrTTSPrivate)
	{
	public:
		_DASVRPTR_CONSTRUCT(IDaSvrTTSPrivate)
		_DASVRPTR_ASSIGN(IDaSvrTTSPrivate)
		...
		_bstr_t _get_InitDisplayName () {BSTR lDisplayName=NULL; if (_DASVRPTR_CHKNULL) GetInterfacePtr()->get_InitDisplayName(&lDisplayName); return _bstr_t(lDisplayName, false);}
		HRESULT _put_InitDisplayName (const _bstr_t& Value) {if (_DASVRPTR_CHKNULL) return GetInterfacePtr()->put_InitDisplayName(Value); else return E_POINTER;}
		_bstr_t _get_InitManufacturer () {BSTR lManufacturer=NULL; if (_DASVRPTR_CHKNULL) GetInterfacePtr()->get_InitManufacturer(&lManufacturer); return _bstr_t(lManufacturer, false);}
		HRESULT _put_InitManufacturer (const _bstr_t& Value) {if (_DASVRPTR_CHKNULL) return GetInterfacePtr()->put_InitManufacturer(Value); else return E_POINTER;}
		SpeechGenderType _get_InitGender () {SpeechGenderType lGender=SpeechGender_Neutral; if (_DASVRPTR_CHKNULL) GetInterfacePtr()->get_InitGender(&lGender); return lGender;}
		HRESULT _put_InitGender (SpeechGenderType Value) {if (_DASVRPTR_CHKNULL) return GetInterfacePtr()->put_InitGender(Value); else return E_POINTER;}
		long _get_InitLanguageID () {long lLanguageID=0; if (_DASVRPTR_CHKNULL) GetInterfacePtr()->get_InitLanguageID(&lLanguageID); return lLanguageID;}
		HRESULT _put_InitLanguageID (long Value) {if (_DASVRPTR_CHKNULL) return GetInterfacePtr()->put_InitLanguageID(Value); else return E_POINTER;}
		_bstr_t _get_InitVersion () {BSTR lVersion=NULL; if (_DASVRPTR_CHKNULL) GetInterfacePtr()->get_InitVersion(&lVersion); return _bstr_t(lVersion, false);}
		HRESULT _put_InitVersion (const _bstr_t& Value) {if (_DASVRPTR_CHKNULL) return GetInterfacePtr()->put_InitVersion(Value); else return E_POINTER;}
		_bstr_t _get_InitString (BSTR pValuePath) {BSTR lString=NULL; if (_DASVRPTR_CHKNULL) GetInterfacePtr()->get_InitString(pValuePath, &lString); return _bstr_t(lString, false);}
		HRESULT _put_InitString (BSTR pValuePath, const _bstr_t& Value) {if (_DASVRPTR_CHKNULL) return GetInterfacePtr()->put_InitString(pValuePath, Value); else return E_POINTER;}
		_bstr_t _get_InitAttribute (BSTR pAttributeName) {BSTR lAttribute=NULL; if (_DASVRPTR_CHKNULL) GetInterfacePtr()->get_InitAttribute(pAttributeName, &lAttribute); return _bstr_t(lAttribute, false);}
		HRESULT _put_InitAttribute (BSTR pAttributeName, const _bstr_t& Value) {if (_DASVRPTR_CHKNULL) return GetInterfacePtr()->put_InitAttribute(pAttributeName, Value); else return E_POINTER;}
		_bstr_t _get_InitFilePath (BSTR pFileId) {BSTR lFilePath=NULL; if (_DASVRPTR_CHKNULL) GetInterfacePtr()->get_InitFilePath(pFileId, &lFilePath); return _bstr_t(lFilePath, false);}
		HRESULT _put_InitFilePath (BSTR pFileId, const _bstr_t& Value) {if (_DASVRPTR_CHKNULL) return GetInterfacePtr()->put_InitFilePath(pFileId, Value); else return E_POINTER;}
...
__if_not_exists(IDaSvrCharacter3Ptr)
{
	_DASVRPTR_DECLARE(IDaSvrCharacter3)
	{
	public:
		_DASVRPTR_CONSTRUCT(IDaSvrCharacter3)
		_DASVRPTR_ASSIGN(IDaSvrCharacter3)
	};
}
