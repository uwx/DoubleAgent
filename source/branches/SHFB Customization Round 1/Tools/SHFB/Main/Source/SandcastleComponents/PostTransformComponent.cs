...
//PATCH
			// The self-branded option is temporary until a permanent solution is found.
			bool selfBranded = true;
			try
			{
				String selfBrandedAttr = nav.GetAttribute ("self-branded", String.Empty);
				if (!String.IsNullOrEmpty (selfBrandedAttr))
				{
					selfBranded = bool.Parse (selfBrandedAttr);
				}
			}
			catch {}
...
//END PATCH
		...
		public override void Apply (XmlDocument document, string key)
		{
			...
			// Adjust the language selector for the current style
			FinalizeLanguageSelector (document);
			...
			InstallColorizerFiles (document);
			PutColorizedCodeBlocks (document);
			FinalizeCopyCode (document);
		...
		#region Colorized Code Blocks
		//=====================================================================

		/// <summary>
		/// Replaces the code blocks that were removed by the CodeBlockComponent
		/// with the colorized code. 
		/// The code blocks are removed to prevent them from being messed up by
		/// the transforms, then reinserted here after the CodeColorizer is done.
		/// </summary>
		/// <param name="document">The document to process.</param>
		private void PutColorizedCodeBlocks (XmlDocument document)
		{
			XmlNode node, codePreTag, parent, codeBlock;
		...
		/// <summary>
		/// This is called to finalize the languageSelector dropdown
		/// </summary>
		/// <param name="document">The document to process.</param>
		private void FinalizeLanguageSelector (XmlDocument document)
		{
			XmlNode node;
			XmlAttribute attr;
		...
		/// <summary>
		/// This is called to finalize the "Copy" code added by the colorizer
		/// </summary>
		/// <param name="document">The document to process.</param>
		private void FinalizeCopyCode (XmlDocument document)
		{
			XmlNodeList codeSpans;
			...
			foreach (XmlNode span in codeSpans)
			{
				...
				else
				{
					...
					var spanCopyImage = span.SelectSingleNode ("img");

					if (spanCopyImage != null)
					{
						spanCopyImage.Attributes.Remove (spanCopyImage.Attributes["src"]);
						spanCopyImage.InnerXml = String.Format (CultureInfo.InvariantCulture,
		...
		/// <summary>
		/// Copies the colorizer style, script and icon files.
		/// </summary>
		private void CopyColorizerFiles ()
		{
			if (outputPaths.Count > 0)
			{
		...
		/// <summary>
		/// Adds the colorizer styles and scripts to the document header.
		/// </summary>
		/// <param name="document">The document to process.</param>
		private void InstallColorizerFiles (XmlDocument document)
		{
			if (!String.IsNullOrEmpty (stylesheet) || !String.IsNullOrEmpty (scriptFile))
			{
				XmlNode head, node;
				XmlAttribute attr;
		...
		/// <summary>
		/// Replaces the CodeColorizer's styles with the native MS Help Viewer styles.
		/// 
		/// This is only done if no colorizer stylesheet is provided.  Not specifying
		/// a colorizer stylesheet implies the help viewers native styles should be used.
		/// </summary>
		/// <param name="codeBlock">The code block to process.</param>
		private void ApplyHelp3CodeStyles (XmlNode codeBlock)
		{
			if (String.IsNullOrEmpty (stylesheet))
			{
				XmlNodeList nodeList;

				nodeList = codeBlock.SelectNodes ("//span[@class='highlight-comment']");
				foreach (XmlNode node in nodeList)
				{
					node.Attributes.GetNamedItem ("class").Value = "comment";
				}
				nodeList = codeBlock.SelectNodes ("//span[@class='highlight-keyword']");
				foreach (XmlNode node in nodeList)
				{
					node.Attributes.GetNamedItem ("class").Value = "keyword";
				}
				nodeList = codeBlock.SelectNodes ("//span[@class='highlight-literal' or @class='highlight-number']");
				foreach (XmlNode node in nodeList)
				{
					node.Attributes.GetNamedItem ("class").Value = "literal";
				}
				nodeList = codeBlock.SelectNodes ("//span[@class='highlight-namespace']");
				foreach (XmlNode node in nodeList)
				{
					node.Attributes.GetNamedItem ("class").Value = "identifier";
				}
				nodeList = codeBlock.SelectNodes ("//span[@class='highlight-inline']");
				foreach (XmlNode node in nodeList)
				{
					node.Attributes.GetNamedItem ("class").Value = "code";
				}
			}
		}
