...
ECHO *
ECHO * With the October 2007 release and on, it is not necessary to create
ECHO * the files unless you install a newer version of the .NET Framework.
ECHO * If you want to recreate it, you must delete the folder first.
ECHO *
PAUSE
GOTO Done
...
ECHO *
ECHO * NOTE: If running this on Vista or Windows 7, you may need to
ECHO *       edit the shortcut that started this script to give it
ECHO *       Run as Administrator priviliges.
ECHO *
PAUSE
...
IF EXIST "%WINDIR%\Microsoft.NET\Framework\v4.0\*.*" SET DNVER=4.0
