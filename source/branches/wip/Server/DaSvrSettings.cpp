	...
	if	(this)
	{
			...
			)
		{
			if	(pAbandonned)
			{
				...
				{
					LogMessage (_DEBUG_ABANDONED, _T("[%p(%d)] DaSvrSettings SKIP CoDisconnectObject"), this, max(m_dwRef,-1));
				}
#endif
			}
			else
...
void DaSvrSettings::OnClientEnded()
{
	...
	{
		LogMessage (_LOG_ABANDONED, _T("[%p(%d)] DaSvrSettings::OnClientEnded"), this, max(m_dwRef,-1));
	}
#endif
