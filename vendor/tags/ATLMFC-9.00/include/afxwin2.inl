...
#else
        {
            // sadaR- Code copied from eVC MFC 3.0
            // Adding this code to eliminate warnings and be compatible with the eVC code.
            (HRGN)prgnUpdate->GetSafeHandle();
            ::InvalidateRect(m_hWnd, lpRectUpdate, flags & RDW_ERASE ? TRUE : FALSE);
            if ((flags & RDW_ERASENOW) || (flags & RDW_UPDATENOW)) 
                ::UpdateWindow(m_hWnd);
            return TRUE;
        }
#endif
...
#else
	//this is implented in atlosapice.h
	::ShowScrollBar(m_hWnd, nBar, bShow); 
#endif // !_WIN32_WCE
}
...
_AFXWIN_INLINE CSplitButton::CSplitButton(CMenu* pMenu)
	...
	{  }
#endif // (_WIN32_WINNT >= 0x600) && defined(UNICODE) && !define(_WIN32_WCE)
